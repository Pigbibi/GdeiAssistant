<template>
    <div class="login">
        ...
        <section class="lists" :class="{'forp4': isSmallScreen}">
            <div v-for="item in foundItems" :key="item.id" class="list">
        ...
        <nav class="main-nav">
            <div>
                <a @click="goToLost"><i class="ibar index"><img
                        src="/img/lostandfound/lost_normal.png"></i>失物</a>
            </div>
            <div>
                <a class="on"><i class="ibar ipublish"><img src="/img/lostandfound/found_selected.png"></i>招领</a>
            </div>
            <div class="publish">
                <div class="pubbtn">
                    <a @click="goToPublish">
...
export default {
    ...
    methods: {
        ...
        loadMoreInfo() {
            if (this.hasMore) {
                this.loadInfo(this.foundItems.length);
            }
        },
        ...
        loadInfo(start) {
            ...
            axios.get(`/api/lostandfound/founditem/start/${start}`)
                .then(response => {
                    this.$toast.clear();
        ...
        goToType(type) {
            this.$router.push(`/lostandfound/foundinfo/type/${type}`);
        },
