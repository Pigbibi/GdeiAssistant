<template>
  <div>
    ...
    <p style="text-indent:28px">
      ...
      XSS攻击通常指的是通过利用网页开发时留下的漏洞，通过巧妙的方法注入恶意指令代码到网页上，使用户加载并执行攻击者恶意制造的网页程序。这些恶意网页程序通常是JavaScript，但实际上也可以包括Java，VBScript，ActiveX，Flash或者甚至是普通的HTML。攻击成功后，攻击者可能得到更高的权限（如执行一些操作）、私密网页内容、会话和Cookie等各种内容。
      应用使用了用户内容可靠输入验证、参数特征匹配检查、Session标记检查、使用HttpOnly的Cookie等技术手段，能有效抵御常见的XSS攻击。
    </p>
    <p style="text-indent:28px">
      （2）SQL注入攻击：SQL注入（英语：SQL
        injection），也称SQL隐码或SQL注码，是发生于应用程序与数据库层的安全漏洞攻击，是代码注入的一种。它允许恶意用户将代码注入到网页上，其他用户在观看网页时就会受到影响。这类攻击通常包含了HTML以及用户端脚本语言。
      XSS攻击通常指的是通过利用网页开发时留下的漏洞，通过巧妙的方法注入恶意指令代码到网页上，使用户加载并执行攻击者恶意制造的网页程序。这些恶意网页程序通常是JavaScript，但实际上也可以包括Java，VBScript，ActiveX，Flash或者甚至是普通的HTML。攻击成功后，攻击者可能得到更高的权限（如执行一些操作）、私密网页内容、会话和Cookie等各种内容。
      应用使用了用户内容可靠输入验证、参数特征匹配检查、Session标记检查、使用HttpOnly的Cookie等技术手段，能有效抵御常见的XSS攻击。
    </p>
    <p style="text-indent:28px">
